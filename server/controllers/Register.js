...
export async function Register(req, res) {
    ...
    try {
        const { name, email, phoneNumber, course, password } = req.body
        ...
        if (exists) {
            ...
            return res.status(400).json({ message: "Please Select the course" });
        } else if (password === "") {
...
export async function login(req, res) {
    try {
        ...
        if (email === "") {
            ...
            return res.status(400).json("Please sing up")
        } else if (password === "") {
            return res.status(400).json({ message: 'please enter your passowrd' })
